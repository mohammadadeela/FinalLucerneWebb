...
import { COLOR_FAMILIES, groupColorsByFamily, productMatchesColorFamily, getColorFamily, normalizeArabic } from "@/lib/colorFamilies";
...
export default function CategoryPage({ title, subtitle, categoryIds, heroImage, heroImagePosition = "center", heroVideo, heroVideoPosition = "50% 50%", defaultSizes, icon: CategoryIcon }: CategoryPageProps) {
  const { data: products, isLoading } = useProductsByCategory(categoryIds);
  const { data: allSubcategories } = useQuery<Subcategory[]>({ queryKey: ["/api/subcategories"] });
  const { t, language } = useLanguage();
  const searchString = useSearch();
  const [location, navigate] = useLocation();
  const urlSubParam = new URLSearchParams(searchString).get("sub");
  const urlColor = new URLSearchParams(searchString).get("color") || "";
  const urlSize  = new URLSearchParams(searchString).get("size")  || "";
  ...
  const subButtonRefs = useRef<Record<number, HTMLButtonElement | null>>({});

  // Resolve ?sub= param — supports both numeric ID and slug
  useEffect(() => {
    if (!urlSubParam) { setActiveSubId(null); return; }
    const asNum = Number(urlSubParam);
    if (!isNaN(asNum) && String(asNum) === urlSubParam) {
      setActiveSubId(asNum);
    } else if (allSubcategories) {
      const found = allSubcategories.find(s => s.slug === urlSubParam);
      setActiveSubId(found ? found.id : null);
    }
  }, [urlSubParam, allSubcategories]);

  // Scroll selected subcategory button into view (for the horizontal strip)
  useEffect(() => {
    if (activeSubId !== null) {
      const btn = subButtonRefs.current[activeSubId];
  ...
  // Apply color/size deep-links (e.g. from the chatbot) when the URL changes
  useEffect(() => {
    setFilters((prev) => ({ ...prev, colors: normalizedUrlColor ? [normalizedUrlColor] : [] }));
  }, [normalizedUrlColor]);
  useEffect(() => {
    setFilters((prev) => ({ ...prev, sizes: urlSize ? [urlSize] : [] }));
  }, [urlSize]);

  const categoryProducts = useMemo(() => products ?? [], [products]);
  ...
  const allSizes = useMemo(() => {
    ...
    categoryProducts.forEach((p) => {
      ...
      else (p.sizes || []).forEach((s: string) => set.add(s));
    });
  ...
  const { minPrice, maxPrice } = useMemo(() => {
    ...
    const prices = categoryProducts.map((p) => parseFloat((p.discountPrice ?? p.price).toString()));
  ...
  const filtered = useMemo(() => {
    ...
    if (activeSubId !== null) result = result.filter((p: any) => {
      const ids: number[] = Array.isArray(p.subcategoryIds) ? p.subcategoryIds : [];
      return ids.includes(activeSubId) || p.subcategoryId === activeSubId;
    });
    ...
    result = result.filter((p) => {
      const price = parseFloat((p.discountPrice ?? p.price).toString());
  ...
  return (
    <div className="min-h-screen flex flex-col">
              ...
              <div className="relative">
                ...
                >
                  {subs.map(sub => (
                    <button
                      key={sub.id}
                      ref={el => { subButtonRefs.current[sub.id] = el; }}
                      type="button"
      ...
      <main ref={productsRef} className="flex-1 w-full px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
