...
export default function Home() {
  ...
  const heroVideoPosition = getSetting(siteSettings, "home_hero_video_position") || "50% 50%";
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        {/* Hero — 3D parallax */}
        <section
          className="relative h-[65vh] sm:h-[78vh] flex items-center justify-center overflow-hidden"
        ...
        {onSale.length > 0 && (
          <section className="bg-background py-12 sm:py-24">
            <div className="w-full px-4 sm:px-6 lg:px-8">
              <div className="flex justify-between items-end mb-10 sm:mb-14">
                <div className="relative">
                  <span className="block text-[10px] uppercase tracking-[0.3em] mb-2 font-medium flex items-center gap-1" style={{ color: "#9B1C1C" }}>
                    <Sparkles className="w-3 h-3" />
                    {language === "ar" ? "عروض حصرية" : "Limited Offers"}
                  </span>
                  <h2 className="font-display text-2xl sm:text-4xl font-semibold mb-2" style={{ color: "#9B1C1C" }} data-testid="text-sales-title">
                    {secOnSaleTitle}
                  </h2>
                  <div className="flex items-center gap-3 mt-3">
                    <span className="block h-px w-10" style={{ backgroundColor: "#9B1C1C" }} />
                          ...
                          <div className="absolute bottom-0 inset-x-0 p-3 sm:p-4 text-white">
                            ...
                            <div className="flex items-center gap-2">
                              {product.discountPrice ? (
                                <>
                                  <span className="text-sm sm:text-base font-bold" style={{ color: "#9B1C1C" }}>₪{Number(product.discountPrice).toFixed(0)}</span>
