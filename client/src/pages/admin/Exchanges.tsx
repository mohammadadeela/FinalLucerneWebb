...
export default function AdminExchanges() {
  ...
  // Exchange window (how many days after delivery a customer can request an exchange)
  const savedExchangeWindowDays = getSetting(siteSettings, "exchange_window_days") || "3";
  const [exchangeWindowDays, setExchangeWindowDays] = useState(savedExchangeWindowDays);
  useEffect(() => { setExchangeWindowDays(savedExchangeWindowDays); }, [savedExchangeWindowDays]);
  const exchangeWindowDaysValid = /^\d+$/.test(exchangeWindowDays.trim()) && Number(exchangeWindowDays) > 0;

  const saveExchangeWindowDays = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", "/api/site-settings", {
        key: "exchange_window_days",
        value: String(Number(exchangeWindowDays)),
      });
      return res.json();
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["/api/site-settings"] });
      toast({ title: ar ? "تم حفظ مهلة الاستبدال" : "Exchange window saved" });
    },
    onError: (e: any) => toast({ title: ar ? "تعذر الحفظ" : "Failed to save", description: e.message, variant: "destructive" }),
  });
  ...
  return (
    <AdminLayout>
      <div className="p-3 sm:p-6 space-y-4 sm:space-y-6" data-testid="page-admin-exchanges">
        ...
        {/* Exchange Window (Days) Editor */}
        <div className="border border-border rounded-xl p-4 bg-card space-y-3" data-testid="section-exchange-window-days">
          <div className="flex items-start gap-2">
            <Clock className="w-4 h-4 mt-0.5 text-muted-foreground flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="font-semibold">{ar ? "مهلة الاستبدال (بالأيام)" : "Exchange Window (Days)"}</p>
              <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
                {ar
                  ? "عدد الأيام بعد استلام الطلب التي يمكن للعميلة خلالها طلب استبدال. الافتراضي 3 أيام."
                  : "How many days after delivery a customer can request an exchange. Default is 3 days."}
              </p>
            </div>
          </div>
          <Input
            ...
            placeholder="3"
            data-testid="input-exchange-window-days"
            className="max-w-[140px]"
          />
          {!exchangeWindowDaysValid && exchangeWindowDays.trim() !== "" && (
            <p className="text-xs text-rose-600">
              {ar ? "الرجاء إدخال عدد أيام صحيح أكبر من صفر" : "Please enter a whole number greater than 0"}
            </p>
          )}
          <div className="flex justify-end">
            <Button
              type="button"
              onClick={() => saveExchangeWindowDays.mutate()}
              disabled={
                saveExchangeWindowDays.isPending ||
                !exchangeWindowDaysValid ||
                Number(exchangeWindowDays) === Number(savedExchangeWindowDays)
              }
              data-testid="button-save-exchange-window-days"
              size="sm"
              className="w-full sm:w-auto"
            >
              {saveExchangeWindowDays.isPending ? <Loader2 className="w-4 h-4 me-2 animate-spin" /> : <Save className="w-4 h-4 me-2" />}
              {ar ? "حفظ المهلة" : "Save Window"}
            </Button>
          </div>
        </div>
      ...
      <Dialog open={!!decisionTarget} onOpenChange={(o) => !o && setDecisionTarget(null)}>
        <DialogContent className="w-[calc(100%-1.5rem)] sm:max-w-md rounded-xl">
          <DialogHeader>
            <DialogTitle>
