...
import { useOrders, useUpdateOrderStatus, useOrder, useAddOrderItem, useUpdateOrderItem, useRemoveOrderItem } from "@/hooks/use-orders";
import { OrderItemEditor, type OrderItemSelection } from "@/components/admin/OrderItemEditor";
import { getOrderItemImage } from "@/lib/utils";
import {
  Table,
  TableBody,
  TableCell,
...
export default function Orders() {
  ...
  // ── Admin order item editing (add / replace / remove) ──────────────────
  const addOrderItem = useAddOrderItem();
  const updateOrderItem = useUpdateOrderItem();
  const removeOrderItem = useRemoveOrderItem();
  const [itemEditorOpen, setItemEditorOpen] = useState(false);
  const [itemEditorMode, setItemEditorMode] = useState<"add" | "replace">("add");
  ...
  const zoneNameMap = useMemo(() => {
    ...
    getShippingZones(siteSettings).forEach(z => {
      map[z.id] = isAr ? (z.nameAr || z.nameEn) : z.nameEn;
    });
    return map;
  }, [siteSettings, isAr]);
        ...
        .map((d: any) => {
          ...
          const itemsHtml = items
            .map((item: any) => {
              ...
              const itemImage = getOrderItemImage(item.product, item.color);
              return `
              <tr>
                <td style="padding:10px 12px;border-bottom:1px solid #f0f0f0;vertical-align:top;">
                  <div style="display:flex;align-items:flex-start;gap:10px;">
                    ${
                      itemImage
                        ? `<img src="${itemImage}" style="width:52px;height:64px;object-fit:cover;border-radius:6px;border:1px solid #e5e7eb;flex-shrink:0;" />`
                    ...
                    <div>
                      ...
                      ${item.product?.barcode ? `<div style="font-family:monospace;font-size:10px;color:#888;margin-top:3px;">⬛ <span style="direction:ltr;unicode-bidi:isolate;display:inline-block;">${item.product.barcode}</span></div>` : ""}
                ...
                <td style="padding:10px 12px;border-bottom:1px solid #f0f0f0;text-align:center;vertical-align:middle;">
                  <span style="font-weight:700;font-size:14px;direction:ltr;unicode-bidi:isolate;display:inline-block;">${item.quantity}</span>
                </td>
                <td style="padding:10px 12px;border-bottom:1px solid #f0f0f0;text-align:right;vertical-align:middle;">
                  ${item.quantity > 1 ? `<div style="font-size:11px;color:#888;margin-bottom:2px;direction:ltr;unicode-bidi:isolate;">₪${unitPrice.toFixed(2)} × ${item.quantity}</div>` : ""}
                  ${
                    isExchange
                      ? `<div><span style="text-decoration:line-through;color:#aaa;font-size:12px;direction:ltr;unicode-bidi:isolate;">₪${lineTotal.toFixed(2)}</span> <strong style="color:#16a34a;direction:ltr;unicode-bidi:isolate;">₪0.00</strong></div>`
                      : `<strong style="font-size:13px;direction:ltr;unicode-bidi:isolate;">₪${lineTotal.toFixed(2)}</strong>`
                  }
                </td>
              </tr>`;
            })
          ...
          <div style="page-break-inside:avoid;border:1px solid #e5e7eb;border-radius:12px;margin-bottom:24px;overflow:hidden;font-family:Arial,sans-serif;box-shadow:0 1px 4px rgba(0,0,0,0.06);">
            ...
            <div style="background:#111;color:#fff;padding:14px 18px;display:flex;justify-content:space-between;align-items:center;gap:12px;">
              <div>
                <div style="font-size:17px;font-weight:800;font-family:monospace;">
                  ${isAr ? "طلب" : "Order"} <span style="direction:ltr;unicode-bidi:isolate;display:inline-block;">#${String(o.id).padStart(6, "0")}</span>
                </div>
                <div style="font-size:11px;color:#aaa;margin-top:3px;">
                  <span style="direction:ltr;unicode-bidi:isolate;display:inline-block;">${o.createdAt ? format(new Date(o.createdAt), "yyyy-MM-dd hh:mm a") : ""}</span>
                </div>
              </div>
            ...
            <div style="padding:14px 18px;background:#fafafa;border-bottom:1px solid #f0f0f0;display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:12px;">
              ...
              <div><span style="color:#888;display:block;font-size:10px;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px;">${isAr ? "الهاتف" : "Phone"}</span><strong style="direction:ltr;unicode-bidi:isolate;display:inline-block;">${o.phone ? (o.phone.startsWith("+") ? o.phone : "+" + o.phone) : ""}${o.phone2 ? " / " + (o.phone2.startsWith("+") ? o.phone2 : "+" + o.phone2) : ""}</strong></div>
            ...
            <div style="padding:12px 18px;border-top:2px solid #e5e7eb;background:#fafafa;">
              <div style="max-width:260px;margin-inline-start:auto;font-size:13px;">
                ${subtotal !== total - shipping + discount ? `<div style="display:flex;justify-content:space-between;color:#666;margin-bottom:4px;"><span>${isAr ? "المجموع الفرعي" : "Subtotal"}</span><span style="direction:ltr;unicode-bidi:isolate;display:inline-block;">₪${subtotal.toFixed(2)}</span></div>` : ""}
                ${shipping > 0 ? `<div style="display:flex;justify-content:space-between;color:#666;margin-bottom:4px;"><span>${isAr ? "الشحن" : "Shipping"}</span><span style="direction:ltr;unicode-bidi:isolate;display:inline-block;">₪${shipping.toFixed(2)}</span></div>` : ""}
                ${discount > 0 ? `<div style="display:flex;justify-content:space-between;color:#16a34a;margin-bottom:4px;"><span>${isAr ? "خصم الاستبدال" : "Exchange discount"}</span><span style="direction:ltr;unicode-bidi:isolate;display:inline-block;">−₪${discount.toFixed(2)}</span></div>` : ""}
                <div style="display:flex;justify-content:space-between;font-weight:800;font-size:15px;padding-top:8px;border-top:2px solid #111;margin-top:4px;">
                  ...
                  <span style="color:${o.paymentMethod === "Exchange" ? "#16a34a" : "#111"};direction:ltr;unicode-bidi:isolate;display:inline-block;">₪${total.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>`;
...
<body>
  ...
  const [deleteConfirmCount, setDeleteConfirmCount] = useState("");
  ...
  const bulkDeleteMutation = useMutation({
    ...
    onSuccess: ({ deleted }) => {
      ...
      toast({
        title: isAr
          ? `تم حذف ${deleted} طلب بنجاح`
          : `${deleted} order(s) deleted successfully`,
      });
      setSelectedIds(new Set());
      setDeleteConfirmOpen(false);
      setDeleteConfirmCount("");
    },
    onError: (err: any) => {
      toast({
        title: isAr ? "فشل الحذف" : "Failed to delete",
        description: err.message,
        variant: "destructive",
      });
    },
  ...
  // the color-specific image for each item, not the generic main image.
  const orderImages = useMemo(() => {
    if (!orderDetails) return [];
    return orderDetails.items
      .map((item) => getOrderItemImage(item.product, item.color))
      .filter((url): url is string => Boolean(url));
  }, [orderDetails]);

  // ── Admin order item editing handlers ───────────────────────────────────
  const openAddItem = () => {
  ...
  const openReplaceItem = (itemId: number) => {
    setItemEditorMode("replace");
    setEditingItemId(itemId);
    setItemEditorOpen(true);
  };

  const editingItem = orderDetails?.items.find((i) => i.id === editingItemId) || null;

  const handleConfirmItemEdit = async (selection: OrderItemSelection) => {
    if (!selectedOrderId) return;
    try {
      if (itemEditorMode === "add") {
        await addOrderItem.mutateAsync({ orderId: selectedOrderId, ...selection });
        toast({ title: isAr ? "تمت إضافة المنتج" : "Product added" });
      } else if (editingItemId) {
        await updateOrderItem.mutateAsync({ orderId: selectedOrderId, itemId: editingItemId, ...selection });
        toast({ title: isAr ? "تم استبدال المنتج" : "Product replaced" });
      }
      setItemEditorOpen(false);
    } catch (err: any) {
      toast({
        title: isAr ? "خطأ" : "Error",
        description: err?.message || (isAr ? "فشلت العملية" : "Operation failed"),
        variant: "destructive",
      });
    }
  };
  ...
  const handleDeleteItem = async () => {
    if (!selectedOrderId || !itemPendingDelete) return;
    try {
      await removeOrderItem.mutateAsync({ orderId: selectedOrderId, itemId: itemPendingDelete.id });
      toast({ title: isAr ? "تم حذف المنتج من الطلب" : "Product removed from order" });
      setItemPendingDelete(null);
    } catch (err: any) {
      const msg = err?.message === "cannot_remove_last_item"
        ? (isAr ? "لا يمكن حذف آخر منتج في الطلب" : "Can't remove the last item in an order")
        : err?.message || (isAr ? "فشل الحذف" : "Failed to remove");
      ...
      <AdminLayout>
        <div className="flex flex-col items-center justify-center py-24 text-center gap-3">
          ...
          <p className="text-muted-foreground font-medium">
            {isAr ? "صفحة الطلبات معطّلة حالياً" : "The orders page is currently disabled"}
          </p>
    ...
    <AdminLayout>
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <span className="text-sm font-semibold text-background/80">
              <span className="ltr-num">₪{selectedTotal.toFixed(2)}</span>
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-1 text-xs text-red-300 hover:text-red-200 transition-colors border border-red-400/30 hover:border-red-400/60 px-2.5 py-1.5 rounded"
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) setDeleteConfirmCount("");
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {isAr ? "حذف الطلبات نهائياً؟" : "Permanently delete orders?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {isAr
                ? `سيتم حذف ${selectedIds.size} طلب نهائياً ولا يمكن التراجع عن هذا الإجراء. سيتم أيضاً حذف عناصر الطلب وطلبات الاستبدال المرتبطة بها.`
                : `${selectedIds.size} order(s) will be permanently deleted. This cannot be undone. Related order items and exchange requests will also be removed.`}
            </AlertDialogDescription>
            <div className="mt-4 rounded-lg border border-red-200 bg-red-50/60 p-3 dark:border-red-900 dark:bg-red-950/20">
              <p className="text-sm font-semibold text-foreground mb-2">
                {isAr
                  ? <>للتأكيد، اكتب عدد الطلبات المحددة: <span className="text-red-600 ltr-num">{selectedIds.size}</span></>
                  : <>To confirm, enter the number of selected orders: <span className="text-red-600 ltr-num">{selectedIds.size}</span></>}
              </p>
              <Input
                type="text"
                inputMode="numeric"
                value={deleteConfirmCount}
                onChange={(e) => setDeleteConfirmCount(e.target.value.replace(/\D/g, ""))}
                placeholder={String(selectedIds.size)}
                className="h-11 bg-background text-center text-lg font-bold ltr-num"
                dir="ltr"
                data-testid="input-confirm-delete-count"
                autoFocus
              />
            </div>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              disabled={bulkDeleteMutation.isPending || Number(deleteConfirmCount) !== selectedIds.size}
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="font-mono font-medium text-sm">
                      <span className="ltr-num">#{o.id.toString().padStart(6, "0")}</span>
                    </TableCell>
                    <TableCell className="whitespace-nowrap">
                      <span className="block text-sm">
                        <span className="ltr-num">{o.createdAt
                          ? format(new Date(o.createdAt), "yyyy-MM-dd")
                          : "—"}</span>
                      </span>
                    ...
                    <TableCell className="font-semibold">
                      <span className="ltr-num">₪{parseFloat(o.totalAmount).toFixed(2)}</span>
                    </TableCell>
        ...
        <DialogContent className="w-[calc(100%-1rem)] sm:max-w-2xl rounded-none max-h-[85svh] sm:max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-display text-xl sm:text-2xl font-mono">
              {t.profile.orderNumber} <span className="ltr-num">#{selectedOrderId?.toString().padStart(6, "0")}</span>
            </DialogTitle>
          </DialogHeader>
          {orderDetails && (
            <div className="space-y-6 mt-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  {orderDetails.linkedUserId && orderDetails.customerEmail && (
                      <Link
                        href={`/admin/users?user=${orderDetails.linkedUserId}`}
                      >
                        <button
                          type="button"
                          className="inline-flex items-center gap-1.5 mt-2 px-2.5 py-1 rounded-md border border-primary/30 bg-primary/5 text-xs font-medium text-primary hover:bg-primary/10 transition-colors"
                          data-testid="button-order-customer-profile"
                        >
                          <UserCircle className="w-3.5 h-3.5 shrink-0" />
                          <span className="ltr-num truncate max-w-[220px]">
                ...
                <div>
                  ...
                  <p className="font-medium"><span className="ltr-num">{orderDetails.order.phone?.startsWith("+") ? orderDetails.order.phone : `+${orderDetails.order.phone}`}</span></p>
                  {orderDetails.order.phone2 && (
                    <p className="font-medium text-muted-foreground"><span className="ltr-num">{orderDetails.order.phone2.startsWith("+") ? orderDetails.order.phone2 : `+${orderDetails.order.phone2}`}</span></p>
                  )}
                </div>
                ...
                {orderDetails.order.shippingRegion && (
                  <div className="sm:col-span-2">
                    <p className="text-xs text-muted-foreground">
                      {isAr ? "منطقة التوصيل" : "Delivery zone"}
                    </p>
                    <p className="font-medium">
                      {zoneNameMap[orderDetails.order.shippingRegion] || orderDetails.order.shippingRegion}
                    </p>
                  </div>
                )}
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">
                    <span className="ltr-num">{orderDetails.order.createdAt
                      ? format(
              ...
              <div className="border-t pt-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold">{t.admin.items}</h3>
                  <button
                    type="button"
                    onClick={openAddItem}
                    ...
                    >
                        ...
                        {itemImage ? (
                          <button
                            ...
                            onClick={() =>
                              setLightbox({
                                images: orderImages,
                                idx: orderImages.indexOf(itemImage),
                              })
                            }
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between">
                        <div>
                              ...
                              <div className="flex flex-wrap items-center gap-x-2 mt-1">
                                {item.product?.id && (
                                  <span className="font-mono text-[10px] text-muted-foreground">
                                    <span className="ltr-num">#{String(item.product.id).padStart(4, "0")}</span>
                                  </span>
                                )}
                                {(item.product as any)?.barcode && (
                                  <span className="inline-flex items-center gap-1 font-mono text-[10px] text-muted-foreground bg-muted/60 px-1.5 py-0.5 rounded">
                                    ...
                                    <span className="ltr-num">{(item.product as any).barcode}</span>
                                  </span>
                                )}
                              </div>
                            </div>
                          ...
                          <div className="flex flex-wrap gap-1.5 mt-2">
                            <span className="inline-flex items-center text-xs bg-secondary rounded px-2 py-0.5 font-medium">
                              {t.admin.qty}: <span className="ltr-num">{item.quantity}</span>
                            </span>
                            {item.size && (
                              <span className="inline-flex items-center text-xs bg-secondary rounded px-2 py-0.5 font-medium">
                        ...
                        <div className="mt-2 text-sm">
                          {item.quantity > 1 && (
                            <p className="text-xs text-muted-foreground">
                              {isAr ? "سعر الوحدة" : "Unit price"}: <span className="ltr-num">₪{parseFloat(item.price).toFixed(2)}</span>
                            </p>
                          )}
                          {orderDetails.order.paymentMethod === "Exchange" ? (
                            <div className="flex items-center gap-2">
                              <span className="text-xs line-through text-muted-foreground">
                                <span className="ltr-num">₪{(parseFloat(item.price) * item.quantity).toFixed(2)}</span>
                              </span>
                              <span className="font-semibold text-emerald-600 dark:text-emerald-400">
                                <span className="ltr-num">₪0.00</span>
                              </span>
                            </div>
                          ) : (
                        ...
                        {/* Admin-only: replace this item's product/color/size/qty, or remove it entirely */}
                        <div className="flex items-center gap-3 mt-2 pt-2 border-t border-border/50">
                          <button
                            type="button"
                            onClick={() => openReplaceItem(item.id)}
                            className="flex items-center gap-1 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors"
                            data-testid={`button-replace-item-${item.id}`}
                          >
                          ...
                          <button
                            type="button"
                            onClick={() => setItemPendingDelete({ id: item.id, name: item.product?.name || "" })}
              ...
              {(() => {
                ...
                const discount = parseFloat(orderDetails.order.discountAmount || "0");
                const credit = parseFloat((orderDetails.order as any).creditUsed || "0");
                const isExchangeDiscount = orderDetails.order.discountCode === "EXCHANGE";
                ...
                return (
                  <div className="border-t pt-4 space-y-1.5 text-sm">
                      ...
                      <div className="flex justify-between text-rose-600 dark:text-rose-400">
                        <span className="flex items-center gap-1.5">
                          {isExchangeDiscount
                            ...
                            : isAr ? "خصم" : "Discount"}
                          {orderDetails.order.discountCode && !isExchangeDiscount && (
                        ...
                        <span className="ltr-num">−₪{discount.toFixed(2)}</span>
                      </div>
                    )}
                    {credit > 0 && (
                      <div className="flex justify-between text-amber-700 dark:text-amber-400">
                        <span>{isAr ? "رصيد نقاط مستخدم" : "Points credit used"}</span>
                        <span className="ltr-num">−₪{credit.toFixed(2)}</span>
                      </div>
                    )}
                    {shipping > 0 && (
                      <div className="flex justify-between text-muted-foreground">
                        <span>{isAr ? "الشحن" : "Shipping"}</span>
                        <span className="ltr-num">₪{shipping.toFixed(2)}</span>
                      </div>
                    )}
                    {(discount + credit) > 0 && (
                      <div className="flex justify-between text-emerald-700 dark:text-emerald-400 text-xs pt-1">
                        <span className="font-semibold">
                          {isAr ? "إجمالي ما وفّره العميل" : "Total customer saved"}
                        </span>
                        <span className="font-bold ltr-num">
                          ₪{(discount + credit).toFixed(2)}
                        </span>
                      </div>
                    )}
                    {/* Admin-only: the discount code and/or loyalty-points
                        credit the customer used. This view is only
                        reachable from the admin panel, so it's never
                        shown to the customer. */}
                    {((discount > 0 && !isExchangeDiscount) || credit > 0) && (
                      <p className="text-[10px] text-muted-foreground flex items-center gap-1 pt-0.5">
                        <Lock className="w-2.5 h-2.5" />
                        {isAr ? "مرئي للمشرف فقط" : "Visible to admin only"}
                      </p>
                    )}
                    <div className="flex justify-between items-center font-bold text-base pt-2 border-t">
                      ...
                      >
                        <span className="ltr-num">₪{total.toFixed(2)}</span>
                      </span>
                    </div>
                  </div>
                );
      ...
      <OrderItemEditor
        ...
        initial={
          itemEditorMode === "replace" && editingItem
            ? {
        ...
        onConfirm={handleConfirmItemEdit}
      />

      <AlertDialog open={!!itemPendingDelete} onOpenChange={(open) => !open && setItemPendingDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{isAr ? "حذف المنتج من الطلب؟" : "Remove this product from the order?"}</AlertDialogTitle>
            <AlertDialogDescription>
              {isAr
                ? `سيتم حذف "${itemPendingDelete?.name || ""}" من الطلب وإعادة الكمية إلى المخزون.`
                : `"${itemPendingDelete?.name || ""}" will be removed from the order and its quantity returned to stock.`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
