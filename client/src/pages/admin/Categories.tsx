...
export default function AdminCategories() {
  ...
  const [subImgUploading, setSubImgUploading] = useState<number | null>(null);
  const fileRef = useRef<HTMLInputElement>(null);
  const newCatFileRef = useRef<HTMLInputElement>(null);
  const catFileRefs = useRef<Record<number, HTMLInputElement | null>>({});
  const subFileRefs = useRef<Record<number, HTMLInputElement | null>>({});

  const [currentCrop, setCurrentCrop] = useState<{ file: File; resolve: (f: File) => void } | null>(null);
  const cropFile = useCallback((file: File): Promise<File> => {
    return new Promise((resolve) => setCurrentCrop({ file, resolve }));
  }, []);
  ...
  const handleCatImageUpload = async (catId: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const url = await handleUpload(cropped);
    if (url) {
  ...
  const handleSubCardImageUpload = async (subId: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const cropped = await cropFile(file);
    setSubImgUploading(subId);
    const url = await handleUpload(cropped);
    if (url) {
      await updateSub.mutateAsync({ id: subId, image: url });
      toast({ title: isAr ? "تم تحديث الصورة" : "Image updated" });
    }
    setSubImgUploading(null);
    const ref = subFileRefs.current[subId];
    if (ref) ref.value = "";
  };
  ...
  const handleToggleShowOnHome = async (sub: Subcategory) => {
    await updateSub.mutateAsync({ id: sub.id, showOnHome: !(sub as any).showOnHome });
  };

  return (
    <AdminLayout>
      <AdminPageHeader
        title={isAr ? "الفئات والتصنيفات الفرعية" : "Categories & Subcategories"}
        description={isAr ? "إدارة الفئات الرئيسية والتصنيفات الفرعية وصورها" : "Manage main categories, subcategories and their images"}
        icon={FolderTree}
        iconGradient="from-emerald-500 to-teal-600"
        testId="text-categories-title"
        actions={
          <Button
      ...
      {catLoading || subLoading ? (
        ...
        <div className="space-y-3">
          {categories?.map((cat) => {
            ...
            return (
              <div key={cat.id} className={`bg-card border border-border overflow-hidden rounded-md transition-all ${isExpanded ? "shadow-md border-foreground/20" : "hover:shadow-sm hover:border-foreground/15"}`} data-testid={`category-section-${cat.id}`}>
                {/* Row */}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        {catSubs.map(sub => (
                          <div key={sub.id} className="border border-border p-3 flex items-center gap-3 bg-background" data-testid={`subcategory-card-${sub.id}`}>
                            <button
                              type="button"
                              className="relative w-12 h-12 rounded-full overflow-hidden border border-border flex-shrink-0 group cursor-pointer"
                              onClick={() => subFileRefs.current[sub.id]?.click()}
                              disabled={subImgUploading === sub.id}
                              title={isAr ? "تغيير الصورة" : "Change image"}
                              data-testid={`button-sub-image-${sub.id}`}
                            >
                              {subImgUploading === sub.id ? (
                                <div className="w-full h-full flex items-center justify-center bg-muted">
                                  <div className="w-4 h-4 rounded-full border-2 border-foreground/20 border-t-foreground animate-spin" />
                                </div>
                              ) : sub.image ? (
                                <>
                                  <img src={sub.image} alt={sub.name} className="w-full h-full object-cover transition-transform duration-200 group-hover:scale-110" />
                                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    <Upload className="w-3.5 h-3.5 text-white" />
                                  </div>
                                </>
                              ) : (
                                <div className="w-full h-full border border-dashed border-border bg-muted/20 flex items-center justify-center group-hover:bg-muted/40 transition-colors">
                              ...
                              <input
                                ...
                                accept="image/*"
                                className="hidden"
                                ref={el => { subFileRefs.current[sub.id] = el; }}
                                onChange={e => handleSubCardImageUpload(sub.id, e)}
                              />
                            </button>
                            ...
                            <div className="flex items-center gap-1.5 flex-shrink-0">
                              <div className="flex flex-col items-center gap-0.5">
                                <button
                                  ...
                                  title={isAr ? "تفعيل / تعطيل" : "Active / Inactive"}
                                ...
                                <span className="text-[9px] text-muted-foreground leading-none">{isAr ? "مفعّل" : "Active"}</span>
                              </div>
                              <div className="flex flex-col items-center gap-0.5">
                                <button
                                  type="button"
                                  onClick={() => handleToggleShowOnHome(sub)}
                                  className={`relative w-9 h-5 rounded-full transition-colors ${(sub as any).showOnHome ? "bg-blue-500" : "bg-gray-300 dark:bg-gray-600"}`}
                                  title={isAr ? "عرض في الصفحة الرئيسية" : "Show on home page"}
                                  data-testid={`toggle-sub-show-home-${sub.id}`}
                                >
                                  <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-all ${(sub as any).showOnHome ? "end-0.5" : "start-0.5"}`} />
                                </button>
                                <span className="text-[9px] text-muted-foreground leading-none">{isAr ? "رئيسية" : "Home"}</span>
                              </div>
              ...
              <div className="flex gap-3 items-center">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <AIGenerateImageButton
                    getPrompt={() => {
                      const name = catForm.name || catForm.nameAr || "";
                      return name ? `Category thumbnail photo for a "${name}" clothing category on a women's fashion boutique website` : "";
                    }}
                    onGenerated={(url) => setCatForm(prev => ({ ...prev, image: url }))}
                    aspect="square"
                    className="rounded-none w-full text-[11px]"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1 space-y-2">
                  ...
                  <AIGenerateImageButton
                    getPrompt={() => {
                      const parentCat = (categories ?? []).find((c: any) => c.id === subForm.categoryId);
                      const parentName = parentCat?.name || parentCat?.nameAr || "";
                      const subName = subForm.name || subForm.nameAr || "";
                      if (!subName) return "";
                      return `Circular thumbnail photo for a "${subName}" subcategory${parentName ? ` (under the "${parentName}" category)` : ""} on a women's fashion boutique website`;
                    }}
                    onGenerated={(url) => setSubForm(prev => ({ ...prev, image: url }))}
                    aspect="square"
                    className="rounded-none w-full text-[11px]"
                  />
                </div>
      ...
      <ImageCropDialog
        file={currentCrop?.file ?? null}
        previewType="category"
        onConfirm={(croppedFile) => {
          const resolver = currentCrop?.resolve;
          setCurrentCrop(null);
        ...
        onCancel={() => {
          const resolver = currentCrop?.resolve;
          setCurrentCrop(null);
          if (currentCrop) resolver?.(currentCrop.file);
        }}
      />
    </AdminLayout>
  );
