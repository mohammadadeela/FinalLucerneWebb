...
function ExchangeHistoryProductCard({
  item,
  tone,
  ar,
}: {
  item: ExchangeDisplayItem;
  tone: "returned" | "new";
  ar: boolean;
}) {
  const isReturned = tone === "returned";
  return (
    <div className={`flex items-center gap-1.5 rounded-lg border p-1.5 min-w-0 ${
      isReturned
        ? "border-amber-200 bg-amber-50/70 dark:bg-amber-950/15"
        : "border-blue-200 bg-blue-50/70 dark:bg-blue-950/15"
    }`}>
      <div className="w-9 h-10 rounded-md overflow-hidden border border-border bg-background shrink-0">
        {item.image ? (
          <img
            src={optimizeCloudinaryUrl(item.image, 120) || item.image}
            alt={item.name}
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-[10px] font-bold leading-tight">
          {item.name || (ar ? `منتج #${item.productId}` : `Product #${item.productId}`)}
        </p>
      ...
      <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[8px] font-black ltr-num ${
        isReturned
...
function parseExchangeInvoiceNote(note: string | null | undefined): ParsedExchangeNote | null {
  ...
  const returnedItems = getSectionItems(
    /(?:القطع المرتجعة|المرتجع|Returned items|Returned)\s*:\s*\n([\s\S]*?)(?=\n(?:رصيد المرتجع|قيمة المرتجع|Return credit)\s*:|$)/i,
  );
  const replacementItems = getSectionItems(
    /(?:القطع البديلة|البديل|Replacement items|Replacement)\s*:\s*\n([\s\S]*?)(?=\n(?:إجمالي القطع البديلة|إجمالي البديل|Replacement total)\s*:|$)/i,
  );

  const returnCredit = parseExchangeMoneyValue(
    safeNote,
    /(?:رصيد المرتجع|قيمة المرتجع|Return credit)\s*:\s*₪?\s*([\d.,]+)/i,
  );
  const replacementTotal = parseExchangeMoneyValue(
    safeNote,
    /(?:إجمالي القطع البديلة|إجمالي البديل|Replacement total)\s*:\s*₪?\s*([\d.,]+)/i,
  );
  const customerPays = parseExchangeMoneyValue(
    safeNote,
    /(?:فرق السعر \(يدفعه الزبون\)|Price difference \(customer pays\))\s*:\s*₪?\s*([\d.,]+)/i,
  );
  const refundValue = parseExchangeMoneyValue(
    safeNote,
    /(?:فرق السعر \(يُرد للزبون\)|Price difference \(refund to customer\))\s*:\s*₪?\s*([\d.,]+)/i,
  );
...
export default function POS() {
  ...
  // Exchange notes are deliberately readable text so old invoices remain
  // understandable, but the note alone cannot show photos, barcodes or a
  // reliable unit price. Reconnect each returned line to the original invoice
  // and current product record for a complete exchange-only detail view.
  const expandedExchangeOriginalOrder = useMemo(() => {
    const originalId = expandedOrderExchangeSummary?.originalInvoiceId;
    if (!originalId) return null;
    return (
      posOrders.find((order: any) => Number(order?.id) === Number(originalId)) ||
      null
    );
  }, [expandedOrderExchangeSummary?.originalInvoiceId, posOrders]);

  const expandedExchangeReturnedItems = useMemo(() => {
    if (!expandedOrderExchangeSummary) return [];
    const originalItems = Array.isArray(expandedExchangeOriginalOrder?.items)
      ? expandedExchangeOriginalOrder.items
      : [];
    const history = Array.isArray(expandedExchangeOriginalOrder?.exchangeHistory)
      ? expandedExchangeOriginalOrder.exchangeHistory
      : [];
    const linkedHistory = history.find(
      (event: any) => Number(event?.sourceOrderId) === Number(expandedOrder?.id),
    );
    const fallbackHistory = linkedHistory || history[history.length - 1];
    const sourceItems: ParsedExchangeNoteItem[] =
      expandedOrderExchangeSummary.returnedItems.length > 0
        ? expandedOrderExchangeSummary.returnedItems
        : (Array.isArray(fallbackHistory?.returnedItems)
            ? fallbackHistory.returnedItems.map((item: any) => {
                ...
                return {
                  ...
                  lineTotal: Number.isFinite(unitPrice) ? unitPrice * quantity : undefined,
                };
              })
            : []);

    return sourceItems.map((noteItem) => {
      const noteProductId = Number(noteItem.productId || 0);
      const originalLine =
        originalItems.find((item: any) => {
          const sameProduct = noteProductId > 0
            ? Number(item?.productId ?? item?.product_id) === noteProductId
            : String(item?.name || "").trim() === noteItem.name.trim();
          if (!sameProduct) return false;
          if (noteItem.size && String(item?.size || "") !== noteItem.size) return false;
          if (noteItem.color && String(item?.color || "") !== noteItem.color) return false;
          return true;
        }) ||
        originalItems.find(
          (item: any) =>
      ...
      const productId = Number(
        noteProductId || originalLine?.productId || originalLine?.product_id || 0,
      );
      const product =
        (products as unknown as Product[]).find(
          (candidate) => Number(candidate.id) === productId,
        ) || null;
      const size = noteItem.size || originalLine?.size || undefined;
      const color = noteItem.color || originalLine?.color || undefined;
      const variants =
        ((product?.colorVariants as ColorVariant[] | undefined) || []);
      const image = product
        ? getProductImage(product, color) || variants[0]?.mainImage || ""
        : String(
            originalLine?.mainImage ||
              originalLine?.image ||
              originalLine?.imageUrl ||
              "",
          );
      const quantity = Math.max(1, Number(noteItem.quantity) || 1);
      const storedUnitPrice = Number(originalLine?.price);
      const parsedLineTotal = Number(noteItem.lineTotal);
      const productUnitPrice = Number(product?.discountPrice || product?.price);
      const unitPrice = Number.isFinite(storedUnitPrice)
        ? storedUnitPrice
        : Number.isFinite(parsedLineTotal)
          ...
          : Number.isFinite(productUnitPrice)
            ? productUnitPrice
            : 0;
      const lineTotal = Number.isFinite(parsedLineTotal)
        ? parsedLineTotal
        : unitPrice * quantity;

      return {
        ...noteItem,
        productId: productId || noteItem.productId,
        name: noteItem.name || originalLine?.name || product?.name || "",
        size,
        color,
        quantity,
        barcode: originalLine?.barcode || product?.barcode || null,
        image,
        unitPrice,
        lineTotal,
      };
    });
  }, [expandedExchangeOriginalOrder, expandedOrder?.id, expandedOrderExchangeSummary, products]);

  const getExchangeDisplayItem = useCallback(
    (rawItem: any, sourceInvoice?: any) => {
      const productId = Number(rawItem?.productId ?? rawItem?.product_id ?? 0);
      const invoiceItems = Array.isArray(sourceInvoice?.items) ? sourceInvoice.items : [];
      const invoiceLine =
        invoiceItems.find(
          (item: any) =>
            Number(item?.productId ?? item?.product_id ?? 0) === productId &&
            String(item?.size || "") === String(rawItem?.size || "") &&
            String(item?.color || "") === String(rawItem?.color || ""),
        ) ||
        invoiceItems.find(
      ...
      const size = rawItem?.size || invoiceLine?.size || undefined;
      const color = rawItem?.color || invoiceLine?.color || undefined;
      ...
      const image = product
        ...
        : String(
            rawItem?.mainImage ||
              rawItem?.image ||
              rawItem?.imageUrl ||
              invoiceLine?.mainImage ||
              invoiceLine?.image ||
              invoiceLine?.imageUrl ||
              "",
          );

      return {
        productId,
        name: String(rawItem?.name || invoiceLine?.name || product?.name || ""),
        quantity: Math.max(1, Number(rawItem?.quantity) || Number(invoiceLine?.quantity) || 1),
        size,
        color,
        image,
      };
  ...
  return (
    <AdminLayout fullscreen={posFullscreen}>
      ...
      {expandedOrder && (
        ...
        >
          <div
            className="bg-background border border-border w-full max-w-md shadow-2xl rounded-2xl overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          ...
          >
            <div className="relative px-5 py-4 border-b border-border bg-muted/30">
              <div className="pe-10">
                ...
                {expandedOrder.note && (
                  isExchangeOrder(expandedOrder.note) && expandedOrderExchangeSummary ? (
                    <div className="mt-2 rounded-xl border border-blue-200 bg-blue-50/70 dark:bg-blue-950/20 p-2.5" data-testid="exchange-note-summary">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <ArrowLeftRight className="w-3.5 h-3.5 shrink-0 text-blue-700 dark:text-blue-300" />
                          <span className="text-[11px] font-bold text-blue-900 dark:text-blue-100">
                            {ar ? "تم التبديل من" : "Exchanged from"}
                          </span>
                          {expandedOrderExchangeSummary.originalInvoiceId && (
                            <button
                              type="button"
                              onClick={() => {
                                if (expandedExchangeOriginalOrder) setExpandedOrder(expandedExchangeOriginalOrder);
                              }}
                              disabled={!expandedExchangeOriginalOrder}
                              className="rounded-full border border-blue-200 bg-background px-2 py-0.5 text-[10px] font-bold text-blue-700 dark:text-blue-300 ltr-num transition-colors enabled:hover:bg-blue-100 disabled:cursor-default"
                              title={expandedExchangeOriginalOrder ? (ar ? "فتح الفاتورة الأصلية" : "Open original invoice") : undefined}
                              data-testid="button-open-original-exchange-invoice"
                            >
                              {ar ? `فاتورة #${expandedOrderExchangeSummary.originalInvoiceId}` : `Invoice #${expandedOrderExchangeSummary.originalInvoiceId}`}
                            </button>
                          )}
                        </div>
                        <span className="shrink-0 text-[9px] font-semibold text-blue-700 dark:text-blue-300">
                          {expandedExchangeReturnedItems.reduce((sum, item) => sum + item.quantity, 0)} {ar ? "قطعة مُعادة" : "returned"}
                        </span>
                      </div>

                      {expandedExchangeReturnedItems.length > 0 ? (
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          {expandedExchangeReturnedItems.map((noteItem, idx) => (
                            <div key={idx} className="flex min-w-0 flex-1 basis-[180px] items-center gap-2 rounded-lg border border-blue-100 bg-background p-1.5">
                              <div className="w-9 h-10 rounded-md overflow-hidden border border-border bg-muted shrink-0">
                                {noteItem.image ? (
                                  <img
                                    src={optimizeCloudinaryUrl(noteItem.image, 120) || noteItem.image}
                                    alt={noteItem.name}
                                    className="w-full h-full object-cover"
                                    loading="lazy"
                                    onError={(event) => {
                              ...
                              <div className="min-w-0 flex-1">
                                <p className="truncate text-[11px] font-bold leading-tight">
                                  {noteItem.name || (ar ? `منتج #${noteItem.productId}` : `Product #${noteItem.productId}`)}
                                </p>
                                {(noteItem.size || noteItem.color) && (
                                  <p className="mt-0.5 truncate text-[9px] text-muted-foreground">
                                    {[noteItem.size, noteItem.color].filter(Boolean).join(" · ")}
                                  </p>
                                )}
                              </div>
                              <span className="shrink-0 rounded-full bg-blue-100 dark:bg-blue-900 px-1.5 py-0.5 text-[9px] font-black text-blue-800 dark:text-blue-100 ltr-num">
                                × {noteItem.quantity}
                              </span>
                            </div>
                          ))}
              ...
              <button
                onClick={() => setExpandedOrder(null)}
                className="absolute top-4 end-5 w-8 h-8 rounded-full flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
                aria-label={ar ? "إغلاق تفاصيل الفاتورة" : "Close invoice details"}
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="max-h-[420px] overflow-y-auto">
              {Array.isArray(expandedOrder.exchangeHistory) && expandedOrder.exchangeHistory.length > 0 && (
                <div className="m-4 mb-2 rounded-xl border border-amber-200 bg-amber-50/60 dark:bg-amber-950/15 p-2.5" data-testid="invoice-exchange-history-detail">
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <div className="flex items-center gap-1.5 text-amber-900 dark:text-amber-100">
                      <ArrowLeftRight className="w-3.5 h-3.5 shrink-0" />
                      <p className="text-[11px] font-bold">
                        {ar ? "تم التبديل منها" : "Exchanged from this invoice"}
                      </p>
                    </div>
                    {expandedOrder.exchangeHistory.length > 1 && (
                      <span className="rounded-full bg-amber-100 dark:bg-amber-900/40 px-2 py-0.5 text-[9px] font-bold text-amber-800 dark:text-amber-100">
                        {expandedOrder.exchangeHistory.length} {ar ? "عمليات" : "exchanges"}
                      </span>
                    )}
                  </div>
                  <div className="space-y-2">
                    {expandedOrder.exchangeHistory.map((event: any, eventIndex: number) => {
                      ...
                      const returnedProducts: ExchangeDisplayItem[] = (Array.isArray(event?.returnedItems) ? event.returnedItems : [])
                        .map((item: any) => getExchangeDisplayItem(item, expandedOrder));
                      const replacementSource =
                        Array.isArray(event?.replacementItems) && event.replacementItems.length > 0
                          ? event.replacementItems
                          : Array.isArray(linkedInvoice?.items)
                            ? linkedInvoice.items
                            : [];
                      const newProducts: ExchangeDisplayItem[] = replacementSource.map((item: any) =>
                        getExchangeDisplayItem(item, linkedInvoice),
                      );

                      return (
                        <div key={eventIndex} className="rounded-xl border border-amber-200 bg-background p-2">
                          <div className="flex items-center justify-between gap-2 mb-2">
                            <div className="flex items-center gap-1.5 flex-wrap min-w-0">
                              {event?.exchangedAt && (
                                <span className="text-[9px] text-muted-foreground ltr-num">
                                  {format(new Date(event.exchangedAt), "yyyy-MM-dd · HH:mm")}
                                </span>
                              )}
                              {event?.override && (
                                <span className="inline-flex items-center gap-1 rounded-full border border-red-200 bg-red-50 px-1.5 py-0.5 text-[8px] font-bold text-red-700 dark:text-red-300">
                            ...
                            {linkedInvoice && (
                              <button
                                type="button"
                                onClick={() => setExpandedOrder(linkedInvoice)}
                                className="inline-flex shrink-0 items-center gap-1 rounded-full border border-blue-200 bg-blue-50 px-2 py-1 text-[9px] font-bold text-blue-700 transition-colors hover:bg-blue-100 dark:bg-blue-950 dark:text-blue-200 ltr-num"
                                title={ar ? "فتح فاتورة التبديل" : "Open exchange invoice"}
                              >
                                <Receipt className="w-3 h-3" />
                                {ar ? `فاتورة #${linkedInvoice.id}` : `Invoice #${linkedInvoice.id}`}
                              </button>
                            )}
                          </div>

                          <div className="grid grid-cols-[minmax(0,1fr)_24px_minmax(0,1fr)] gap-1.5 items-start">
                            <div className="min-w-0">
                              <p className="mb-1 text-[9px] font-bold text-amber-800 dark:text-amber-200">
                                {ar ? "المنتج المُعاد" : "Returned"}
                              </p>
                              <div className="space-y-1">
                                {returnedProducts.map((item, itemIndex) => (
                                  <ExchangeHistoryProductCard key={itemIndex} item={item} tone="returned" ar={ar} />
                                ))}
                              </div>
                            </div>

                            <div className="flex items-center justify-center pt-6 text-blue-600 dark:text-blue-300">
                              <ArrowLeftRight className="w-4 h-4" />
                            </div>

                            <div className="min-w-0">
                              <p className="mb-1 text-[9px] font-bold text-blue-700 dark:text-blue-300">
                                {ar ? "المنتج الجديد" : "New product"}
                              </p>
                              <div className="space-y-1">
                                {newProducts.length > 0 ? (
                                  newProducts.map((item: ExchangeDisplayItem, itemIndex: number) => (
                                    <ExchangeHistoryProductCard key={itemIndex} item={item} tone="new" ar={ar} />
                                  ))
                                ) : (
                                  <div className="rounded-lg border border-dashed border-blue-200 px-2 py-3 text-center text-[8px] text-muted-foreground">
                                    {ar ? "تفاصيل المنتج غير متوفرة" : "Product details unavailable"}
                                  </div>
                                )}
                              </div>
                            </div>
              ...
              {(expandedOrder.items || []).length === 0 ? (
                ...
                <div className="divide-y divide-border">
                  {(expandedOrder.items || []).map((item: any, i: number) => {
                    const isExchangeDetail = isExchangeOrder(expandedOrder.note);
                    const itemProductId = Number(item.productId ?? item.product_id ?? 0);
                    const matchedProduct = products.find(
                      (p: Product) =>
                        isExchangeDetail
                          ? Number(p.id) === itemProductId
                          : p.id === (item.productId || item.product_id),
                    );
                    const matchedVariants =
                      ((matchedProduct?.colorVariants as ColorVariant[] | undefined) || []);
                    const imgSrc = matchedProduct
                      ? item.color
                        ? matchedVariants.find((cv) => cv.name === item.color)?.mainImage ||
                          matchedProduct.mainImage ||
                          (isExchangeDetail ? matchedVariants[0]?.mainImage : "")
                        : matchedProduct.mainImage ||
                          (isExchangeDetail ? matchedVariants[0]?.mainImage : "")
                      : isExchangeDetail
                      ...
                      >
                        <div className="relative flex-shrink-0">
                          <div className="w-14 h-16 rounded-lg overflow-hidden bg-muted border border-border">
                            {imgSrc ? (
                              <img
                                src={isExchangeDetail ? optimizeCloudinaryUrl(imgSrc, 180) || imgSrc : imgSrc}
                                alt={item.name}
                                className="w-full h-full object-cover"
                                loading="lazy"
                                onError={(event) => {
