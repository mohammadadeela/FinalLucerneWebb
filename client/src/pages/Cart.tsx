...
function CartItemImage({ item, soldOut, sizeClass, labelSize, language }: {
  item: CartItem;
  soldOut: boolean;
  sizeClass: string;
  labelSize: string;
  ...
  const optimizedSrc = optimizeCloudinaryUrl(imgUrl, 300) || imgUrl;
  const blurSrc = blurCloudinaryUrl(imgUrl);

  return (
    <div className={`relative ${sizeClass} aspect-[3/4] bg-secondary overflow-hidden rounded-2xl group`}>
      {blurSrc ? (
        <img
          src={blurSrc}
          aria-hidden
          className={`absolute inset-0 z-30 w-full h-full object-cover pointer-events-none transition-opacity duration-700 scale-110 ${imageReady ? "opacity-0" : "opacity-100"}`}
        />
      ) : (
        <div className={`absolute inset-0 z-30 bg-muted transition-opacity duration-500 pointer-events-none ${imageReady ? "opacity-0" : "animate-pulse opacity-100"}`} />
      )}
      <img
        src={optimizedSrc}
        alt={item.product.name}
        className={`w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 ${soldOut ? "grayscale" : ""}`}
        onLoad={() => setImageReady(true)}
        onError={() => setImageReady(true)}
      />
      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all duration-400 z-10 pointer-events-none" />
      {soldOut && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/40 z-20">
          <span className={`text-white font-bold uppercase bg-red-600 px-2 py-0.5 ${labelSize}`}>
...
export default function Cart() {
  ...
  // Scroll to the item the user tapped on in the "added to cart" notification
  const itemRefs = useRef<Record<string, HTMLDivElement | null>>({});
  const scrolledToHighlightRef = useRef(false);

  useEffect(() => {
    if (scrolledToHighlightRef.current || items.length === 0) return;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="space-y-6">
              {items.map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    ref={(el) => { itemRefs.current[makeKey(item.product.id, item.size, item.color)] = el; }}
                    className={`relative border-b border-border pb-6 md:pb-0 md:border-0 ${soldOut ? "opacity-60" : ""}`}
                  ...
                  >
                    ...
                    <div className="hidden md:grid md:grid-cols-6 gap-4 items-center">
                      <div className="col-span-3 flex items-center gap-6">
                        <Link href={`/product/${item.product.id}`} className="flex-shrink-0">
                          <CartItemImage item={item} soldOut={soldOut} sizeClass="w-24" labelSize="text-[10px] tracking-wider" language={language} />
                        </Link>
                        <div>
                    ...
                    <div className="flex md:hidden gap-3">
                      <Link href={`/product/${item.product.id}`} className="flex-shrink-0">
                        <CartItemImage item={item} soldOut={soldOut} sizeClass="w-20" labelSize="text-[8px]" language={language} />
                      </Link>
          ...
          <div className="col-span-1">
            <div className="bg-secondary p-6 sm:p-8 sticky top-28">
              ...
              <div className="border-t border-border pt-4 mb-4 flex justify-between items-center text-lg font-medium">
                <span>{t.cart.total}</span>
                <span data-testid="text-cart-total">₪{availableTotal.toFixed(2)}</span>
              </div>

              {loyaltyPointsEnabled && availableTotal > 0 && Math.floor(availableTotal / 2) > 0 && (
                <div
                  className="mb-6 relative overflow-hidden rounded-xl p-3.5 bg-gradient-to-r from-amber-500 via-rose-500 to-pink-600 text-white shadow-md"
                  data-testid="text-cart-points-preview"
                >
                  ...
                  <div className="relative flex items-center gap-3">
                    ...
                    <div className="flex-1 leading-snug">
                      <p className="text-sm font-bold">
                        {language === "ar"
                          ? `ستحصلين على ${Math.floor(availableTotal / 2)} نقطة إضافية عند تسليم طلبك!`
                          : `You'll earn ${Math.floor(availableTotal / 2)} extra points when your order is delivered!`}
                      </p>
                    </div>
                  </div>
                </div>
