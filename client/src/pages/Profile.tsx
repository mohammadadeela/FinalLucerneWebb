...
import { Package, User, LogOut, ExternalLink, Clock, Truck, PackageCheck, XCircle, CheckCircle2, Search, X, Eye, Sparkles, Gift, Shield, ArrowLeft, ArrowRight, Mail, Phone, Settings, ArrowLeftRight } from "lucide-react";
...
import { getOrderItemImage, isPlaceholderEmail } from "@/lib/utils";
...
export default function Profile() {
  ...
  const exchangeWindowDays = (() => {
    const raw = siteSettings?.exchange_window_days;
    const n = raw ? parseFloat(raw) : NaN;
    return Number.isFinite(n) && n > 0 ? n : 3;
  })();
  const EXCHANGE_WINDOW_MS = exchangeWindowDays * 24 * 60 * 60 * 1000;
  ...
  const isDressProduct = (item: any) => {
    ...
    const subIds: number[] = Array.isArray(product.subcategoryIds) ? product.subcategoryIds : [];
    const allSubIds = product.subcategoryId != null
      ? Array.from(new Set([...subIds, Number(product.subcategoryId)]))
      : subIds;
    return (
      (product.categoryId != null && excludedCategoryIds.has(Number(product.categoryId))) ||
      allSubIds.some((id) => excludedSubcategoryIds.has(Number(id)))
    );
  };
  const isItemExchangeable = (order: any, item: any): { ok: boolean; reason?: string } => {
    ...
    // Exchange orders cannot be exchanged again
    if (order.paymentMethod === "Exchange") return { ok: false, reason: ar ? "لا يمكن استبدال طلب استبدال" : "Exchange orders cannot be exchanged again" };
    ...
    const orderTime = new Date(order.deliveredAt ?? order.createdAt).getTime();
    if (isNaN(orderTime) || Date.now() - orderTime > EXCHANGE_WINDOW_MS)
      return { ok: false, reason: ar ? `انتهت مهلة الاستبدال (${exchangeWindowDays} ${exchangeWindowDays === 1 ? "يوم" : "أيام"})` : `Exchange window expired (${exchangeWindowDays} day${exchangeWindowDays === 1 ? "" : "s"})` };
            ...
            <div className="relative overflow-hidden rounded-[1.75rem] border border-border bg-gradient-to-br from-secondary via-secondary to-background p-5 sm:p-6 shadow-sm">
              ...
              <div className="relative flex items-start gap-4 mb-5">
                ...
                <div className="min-w-0 flex-1 space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1.5 mt-0.5 truncate" dir="ltr">
                    ...
                    <div className="relative p-6">
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-5">
                        ...
                        <div className="text-end">
                          ...
                          <p className="text-3xl font-bold" data-testid="text-loyalty-credit"><span className="ltr-num">₪{credit.toFixed(2)}</span></p>
                        </div>
                      </div>

                      {/* Progress bar */}
                      <div className="mb-2 flex items-center justify-between text-[11px] text-white/80">
                        <span className="ltr-num">{cyclePoints} / 450</span>
                  ...
                  <div className="relative group">
                    ...
                    {searchQuery && (
                      <button
                        ...
                        className="absolute end-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center bg-black hover:bg-neutral-800 transition-all shadow-sm"
                      ...
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-3">
                          <div className="inline-flex items-center rounded-full border border-border/60 bg-background px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">
                            {t.profile.orderNumber} <span className="ltr-num">#{order.id.toString().padStart(6, '0')}</span>
                          </div>
                          <div className="space-y-1">
                            <p className="font-semibold text-base"><span className="ltr-num">{order.createdAt ? format(new Date(order.createdAt), 'yyyy-MM-dd') : 'N/A'}</span></p>
                        ...
                        <div className="flex flex-col gap-3 sm:items-end">
                          ...
                          <p className="font-bold text-lg"><span className="ltr-num">₪{parseFloat(order.totalAmount).toFixed(2)}</span></p>
                        </div>
                      </div>
            ...
            {activeTab === "exchanges" && (
              <div id="exchanges-section">
                ...
                {exchangeSubTab === "eligible" && exchangesEnabled && (() => {
                  const EXCHANGE_WINDOW_MS_LOCAL = EXCHANGE_WINDOW_MS;
                  const deliveredOrders = (orders || []).filter(o => {
                    if (o.userId !== user.id) return false;
                    if (o.status !== "Delivered") return false;
                    // Window is measured from the delivery date (تم التسليم), not the order date.
                    const orderTime = new Date((o as any).deliveredAt ?? (o as any).createdAt).getTime();
                  ...
                  const sorted = eligibleSort === "all"
                    ...
                    : [...deliveredOrders].sort((a, b) => {
                        const ta = new Date((a as any).deliveredAt ?? (a as any).createdAt).getTime();
                        const tb = new Date((b as any).deliveredAt ?? (b as any).createdAt).getTime();
                              ...
                              <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                  <p className="text-sm font-medium">
                                    {ar ? "طلب رقم" : "Order"} <span className="ltr-num">#{String(o.id).padStart(6, "0")}</span>
                                  </p>
            ...
            {activeTab === "recently_viewed" && (
              <div>
                ...
                {productsLoading ? (
                  ...
                  <div className="border border-border p-8 text-center bg-card rounded-2xl">
                    ...
                    <Link href="/shop">
                      <Button variant="outline" className="rounded-full uppercase tracking-widest text-sm" data-testid="button-browse-shop">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {recentlyViewed.map((p) => (
                      <div key={p.id} data-testid={`card-recently-viewed-${p.id}`}>
            ...
            <div className="overflow-y-auto flex-1 px-4 sm:px-6 py-4 sm:py-5 space-y-4 sm:space-y-5">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm text-muted-foreground">
                  <span className="ltr-num">{orderDetails.order.createdAt ? format(new Date(orderDetails.order.createdAt), 'yyyy-MM-dd') : ''}</span>
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium text-sm"><span className="ltr-num">{orderDetails.order.phone?.startsWith("+") ? orderDetails.order.phone : `+${orderDetails.order.phone}`}</span></p>
                  {orderDetails.order.phone2 && (
                    <p className="font-medium text-sm text-muted-foreground"><span className="ltr-num">{orderDetails.order.phone2.startsWith("+") ? orderDetails.order.phone2 : `+${orderDetails.order.phone2}`}</span></p>
                  )}
                </div>
                    ...
                    return (
                      ...
                      >
                        {getOrderItemImage(item.product, item.color) && (
                          <ProductThumb src={getOrderItemImage(item.product, item.color)!} className="w-12 h-16 rounded-sm" hover />
                        )}
                        <div className="flex-1 min-w-0">
                          <p className={`font-medium truncate ${productId ? "hover:underline" : ""}`}>{item.product?.name ?? (ar ? "منتج محذوف" : "Deleted product")}</p>
                          <div className="flex flex-wrap gap-2 mt-1">
                            <span className="text-xs text-muted-foreground">{t.profile.qty}: <span className="ltr-num">{item.quantity}</span></span>
                            {item.size && (
                        ...
                        <p className="font-medium flex-shrink-0"><span className="ltr-num">₪{(parseFloat(item.price) * item.quantity).toFixed(2)}</span></p>
                      </div>
                    );
                  })}
                </div>
                    ...
                    <div className="mt-4 rounded-xl border border-pink-200 dark:border-pink-800/60 bg-pink-50/60 dark:bg-pink-900/10 overflow-hidden" data-testid="section-exchange-options">
                      ...
                      <div className="p-3 space-y-2">
                        {eligibleItems.map(({ it }) => (
                            ...
                            onClick={(e) => {
                              ...
                              setExchangeItem({
                                ...
                                items: eligibleItems.map(({ it: ei }) => ({
                                  ...
                                  size: ei.size,
                                  color: ei.color,
                                  image: getOrderItemImage(ei.product, ei.color),
                                })),
                                clickedItemId: it.id,
                              });
                            }}
                          ...
                          >
                            {getOrderItemImage(it.product, it.color) && (
                              <ProductThumb src={getOrderItemImage(it.product, it.color)!} className="w-10 h-12 rounded" hover />
                            )}
                            <div className="flex-1 min-w-0">
                        ...
                        {alreadyRequestedItems.map(({ it }) => (
                          ...
                          >
                            {getOrderItemImage(it.product, it.color) && (
                              <ProductThumb src={getOrderItemImage(it.product, it.color)!} className="w-10 h-12 rounded" grayscale />
                            )}
                            <div className="flex-1 min-w-0">
                  ...
                  <div className="border-t pt-4 space-y-2 text-sm">
                    ...
                    {discount > 0 && (
                      <div className="flex justify-between text-rose-600 dark:text-rose-400">
                        ...
                        <span className="font-medium ltr-num">−₪{discount.toFixed(2)}</span>
                      </div>
                    )}
                    {credit > 0 && (
                      <div className="flex justify-between text-amber-700 dark:text-amber-400">
                        <span>{ar ? "رصيد مستخدم" : "Credit applied"}</span>
                        <span className="font-medium ltr-num">−₪{credit.toFixed(2)}</span>
                      </div>
                    )}
                    {shipping > 0 && (
                      <div className="flex justify-between text-muted-foreground">
                    ...
                    {(discount + credit) > 0 && (
                      <div className="flex justify-between text-emerald-700 dark:text-emerald-400 text-xs pt-1">
                        ...
                        <span className="font-bold ltr-num">₪{(discount + credit).toFixed(2)}</span>
                      </div>
                    )}
                    <div className="flex justify-between items-center pt-2 border-t">
                      <p className="font-semibold text-base">{t.checkout.total}</p>
                      <p className="text-lg font-bold" data-testid="text-order-detail-total"><span className="ltr-num">₪{total.toFixed(2)}</span></p>
                    </div>
                  </div>
                );
              })()}
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-3xl font-bold text-emerald-700 dark:text-emerald-300 leading-none" data-testid="text-convert-preview-credit">
                            <span className="ltr-num">₪{willGetCredit}</span>
                          </p>
                        </div>
                        ...
                        <div className="flex-1 min-w-0 text-end">
                          ...
                          <p className="text-xl font-bold text-emerald-700 dark:text-emerald-300 leading-none">
                            <span className="ltr-num">{willConvert}</span> <span className="text-xs font-medium text-emerald-700/70 dark:text-emerald-300/70">{ar ? "نقطة" : "pts"}</span>
                          </p>
                        </div>
                      </div>
                      {remainder > 0 && (
