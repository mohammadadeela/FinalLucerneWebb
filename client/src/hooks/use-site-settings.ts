...
const STORAGE_KEY = "lucerne_site_settings_v1";
const QUERY_KEY = ["/api/site-settings"];

/** Read cached settings from localStorage (instant — no network) */
...
/** Persist fresh settings to localStorage for next page load */
...
export function useSiteSettings() {
  ...
  const query = useQuery<Record<string, string>>({
    queryKey: QUERY_KEY,
    staleTime: 1000 * 60 * 5,           // 5 min — revalidate in background
    gcTime:    1000 * 60 * 60 * 24,      // 24 h — keep in memory all session
    placeholderData: cached ?? defaultSettings, // show instantly from cache/defaults
  });
...
export const defaultSettings: Record<string, string> = {
  ...
  home_hero_video_position: "50% 50%",
  ...
  dresses_hero_video_position: "50% 50%",
  ...
  clothes_hero_video_position: "50% 50%",
  ...
  shoes_hero_video_position: "50% 50%",
  ...
  sales_hero_video_position: "50% 50%",
  ...
  reports_page_enabled: "true",

  exchange_note: "الفساتين والالبسة الرسمية لا تبدل",
