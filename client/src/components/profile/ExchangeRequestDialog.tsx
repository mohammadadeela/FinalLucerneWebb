  ...
  const availableSizes = useMemo<string[]>(() => {
    ...
    if (variants.length > 0) {
      if (selection.color) {
        ...
        if (!v) return [];
        return sortSizes((v.sizes ?? []).filter((s) => (v.sizeInventory?.[s] ?? 0) > 0));
      }
    ...
    return sortSizes(sizes.filter((s) => (inv[s] ?? 0) > 0));
    ...
    onError: (e: any) => {
      ...
      const known: Record<string, [string, string]> = {
        ...
        exchange_window_expired: [
          `انتهت مهلة الـ ${exchangeWindowDays} ${exchangeWindowDays === 1 ? "يوم" : "أيام"} للاستبدال`,
          `The ${exchangeWindowDays}-day exchange window has expired`,
        ],
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription className="text-xs sm:text-sm">
            {ar
              ? `اختاري المنتجات التي تريدين استبدالها وحددي المقاس المطلوب لكل منتج. يمكن الاستبدال خلال ${exchangeWindowDays} ${exchangeWindowDays === 1 ? "يوم" : "أيام"} من الاستلام.`
              : `Select the items you want to exchange and choose a preferred size for each. Exchange is accepted within ${exchangeWindowDays} day${exchangeWindowDays === 1 ? "" : "s"} of delivery.`}
          </DialogDescription>
        </DialogHeader>
