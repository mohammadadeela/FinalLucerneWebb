...
export function AdminLayout({
  ...
  /**
   * When true, hides the sidebar and top bar entirely so the page (used by
   * the POS screen) can take over the full browser viewport for a clean,
   * professional in-store checkout look. The page itself is responsible for
   * providing its own way to exit fullscreen (e.g. a small button), since
   * AdminLayout renders none of its own chrome in this mode.
   */
  fullscreen?: boolean;
}) {
    ...
    <>
      ...
      <div className="p-3 border-t border-border/60 space-y-2">
        ...
        <Button
          variant="ghost"
          size="sm"
          className="dark-red-zone w-full justify-start text-destructive hover:text-destructive hover:bg-destructive/10"
  ...
  // Fullscreen mode (used by POS): no sidebar, no top bar — the page owns
  // the entire viewport for a clean, professional in-store checkout look.
  if (fullscreen) {
    return (
      <div className="h-screen w-screen overflow-hidden bg-muted/30 flex flex-col">
        ...
        <div className="h-14 bg-card/95 backdrop-blur border-b border-border/60 flex items-center px-4 sm:px-6 justify-between sticky top-0 z-30">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              variant="ghost"
              size="sm"
              className="dark-red-zone gap-2 text-destructive hover:text-destructive hover:bg-destructive/10"
              onClick={() => logout.mutate()}
