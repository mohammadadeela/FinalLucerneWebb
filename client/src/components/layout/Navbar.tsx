...
export function DressIcon({ className }: { className?: string }) {
  return (
    ...
    >
      <path
        d="M53.9,19.43c-0.29-0.02-0.56-0.09-0.8-0.21c-0.39-0.08-0.8-0.13-1.22-0.14c-2.72-0.11-5.97,1.14-8.99,4.4
...
export function HeelIcon({ className }: { className?: string }) {
  return (
    <svg
      className={className}
...
function ClickIcon({
  ...
  return (
    ...
    >
      <span
        style={{ display: "inline-flex", animation: anim }}
...
export function Navbar() {
  ...
  const [searchHistory, setSearchHistory] = useState<string[]>(() => {
    try {
      return JSON.parse(localStorage.getItem("lucerne_search_history") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const shownNotifIds = useRef<Set<number>>(new Set());
  const initialNotifLoad = useRef(false);
  const { data: notificationsData = [] } = useQuery<any[]>({
    queryKey: ["/api/notifications"],
    enabled: !!user,
    // Poll once a minute, and pause when the tab is in the background.
    // Previously polled every 10s for every signed-in visitor (6 req/min/user),
    // which was a major source of server CPU load on Hostinger.
    refetchInterval: (q) => (document.visibilityState === "visible" ? 60_000 : false),
  ...
  // Fire in-app toasts + native browser notifications for new unread updates
  useEffect(() => {
    if (!user || notificationsData.length === 0) return;
    if (!initialNotifLoad.current) {
      // On first load: silently absorb already-READ notifications so they don't
      // re-toast, but leave UNREAD ones so offline customers see them on return.
      notificationsData.forEach((n: any) => {
        if (n.isRead) shownNotifIds.current.add(n.id);
      });
      initialNotifLoad.current = true;
    }
    const newUnread = notificationsData.filter(
      (n: any) => !n.isRead && !shownNotifIds.current.has(n.id),
    );
    if (newUnread.length === 0) return;
    newUnread.forEach((n: any) => {
      shownNotifIds.current.add(n.id);
      const body = language === "ar" ? n.messageAr : n.message;
      const link = n.link || "/profile";
      const isAr = language === "ar";

      // Derive toast appearance from notification type
      const type = n.type as string | undefined;
      const isExchange = type?.startsWith("exchange_");
      const isApproved = type === "exchange_approved";
      const isDenied = type === "exchange_denied";
      const isCancelled = type === "order_cancelled";
      const isShipped = type === "order_shipped";
      const isDelivered = type === "order_delivered";
      const isProcessing = type === "order_processing";

      let title: string;
      if (isAr) {
        if (isApproved)       title = "تمت الموافقة على الاستبدال ✓";
        else if (isDenied)    title = "تم رفض طلب الاستبدال";
        else if (isCancelled) title = "تم إلغاء الطلب";
        else if (isShipped)   title = "طلبك في الطريق 🚚";
        else if (isDelivered) title = "تم تسليم طلبك ✓";
        else if (isProcessing) title = "طلبك قيد المعالجة";
        else                  title = "تحديث طلبك";
      } else {
        if (isApproved)       title = "Exchange Approved ✓";
        else if (isDenied)    title = "Exchange Request Declined";
        else if (isCancelled) title = "Order Cancelled";
        else if (isShipped)   title = "Order Shipped 🚚";
        else if (isDelivered) title = "Order Delivered ✓";
        else if (isProcessing) title = "Order Being Processed";
        else                  title = "Order Update";
      }

      const icon = isExchange ? "exchange" : isCancelled ? "error" : "success";
      const variant = isCancelled ? "destructive" : undefined;
      ...
      if ("Notification" in window && Notification.permission === "granted") {
        const notif = new window.Notification("Lucerne Boutique 🛍", {
          body,
          icon: "/favicon.ico",
        });
        notif.onclick = () => {
  ...
  const newsBarEnabled =
    getSetting(siteSettings, "news_bar_enabled") === "true";
  ...
  const productMatchesColor = (p: any, q: string): boolean => {
    ...
    for (const fam of COLOR_FAMILIES) {
      ...
      if (famMatches) {
        if (colorVariants.some((v) => (v.colorTags || []).includes(fam.key)))
          return true;
  ...
  const searchResults = useMemo(() => {
    ...
    // Price search: supports "50", "<50", "<=50", ">20", ">=20", "20-50", "20 to 50", "₪50", "50nis"
    const priceQ = q.replace(/[₪$,]|nis|شيكل/gi, "").trim();
    let priceFilter: ((price: number) => boolean) | null = null;
    const rangeMatch = priceQ.match(
      /^(\d+(?:\.\d+)?)\s*(?:-|to|إلى|الى)\s*(\d+(?:\.\d+)?)$/i,
    );
    const cmpMatch = priceQ.match(/^(<=|>=|<|>)\s*(\d+(?:\.\d+)?)$/);
    const exactMatch = priceQ.match(/^(\d+(?:\.\d+)?)$/);
    if (rangeMatch) {
      const lo = parseFloat(rangeMatch[1]);
      const hi = parseFloat(rangeMatch[2]);
      const [min, max] = lo <= hi ? [lo, hi] : [hi, lo];
      priceFilter = (price) => price >= min && price <= max;
    } else if (cmpMatch) {
      const op = cmpMatch[1];
      ...
      // Tolerance scales with the searched amount: ±20% but at least ±25 NIS,
      // so a search for "100" finds ~75–125 and "300" finds ~240–360.
      const tolerance = Math.max(25, val * 0.2);
      priceFilter = (price) =>
        price >= val - tolerance && price <= val + tolerance;
    }

    const getEffectivePrice = (p: any) =>
      Number(p.discountPrice || p.price || 0);

    // Build set of category/subcategory IDs whose name (en/ar) matches the query.
    const matchesTaxonomy = (item: any) => {
      if (!item) return false;
      return (
        (item.name || "").toLowerCase().includes(lower) ||
        normalizeArabic(item.nameAr || item.name_ar || "").includes(normQ) ||
        (item.slug || "").toLowerCase().includes(lower)
      );
    };
    const matchedSubcatIds = new Set<number>(
      (allSubcategories || []).filter(matchesTaxonomy).map((s: any) => Number(s.id)),
    );
    const matchedCatIds = new Set<number>(
      (allCategories || []).filter(matchesTaxonomy).map((c: any) => Number(c.id)),
    );

    const productMatchesTaxonomy = (p: any): boolean => {
      if (matchedCatIds.has(Number(p.categoryId))) return true;
      if (p.subcategoryId != null && matchedSubcatIds.has(Number(p.subcategoryId))) return true;
      const ids: any[] = Array.isArray(p.subcategoryIds) ? p.subcategoryIds : [];
      for (const id of ids) if (matchedSubcatIds.has(Number(id))) return true;
      return false;
    };

    const matched = (allProducts || []).filter((p: any) => {
      if (priceFilter && priceFilter(getEffectivePrice(p))) return true;
      return (
  ...
  }, [searchQuery, allProducts, allCategories, allSubcategories]);
  ...
  const resolveColorCode = (variant: any): string => {
    ...
    for (const family of COLOR_FAMILIES) {
      for (const member of family.members) {
        if (member.nameEn.toLowerCase() === lower || member.nameAr === name)
          return member.hex;
      }
      if (family.nameEn.toLowerCase() === lower || family.nameAr === name)
        return family.members[0]?.hex || "#9ca3af";
    }
    return "#9ca3af";
  };
  ...
  const updateNewsBarOffset = useCallback(() => {
    const h =
      showNewsBar && newsBarRef.current ? newsBarRef.current.offsetHeight : 0;
  ...
  return (
    <>
      ...
      {showNewsBar &&
        (() => {
          const items = newsBarText
            .split("|")
            .map((s) => s.trim())
            .filter(Boolean);
          // The strip below repeats this items list 4x per group to keep
          // it filling ultra-wide screens with no gaps, and the animation
          // moves exactly one group's width (50%) per cycle — so a fixed
          // duration made the scroll speed up the more text an admin
          // configured (more items = more distance covered in the same
          // time). Scaling the duration to the total text length keeps a
          // comfortable, constant reading pace no matter how much text is
          // in the bar, with a floor so a short message never rushes by.
          const newsBarCharCount =
            items.reduce((sum, s) => sum + s.length, 0) || 1;
          const newsBarDuration = Math.max(20, newsBarCharCount * 0.45);
          return (
            <div
              ref={newsBarRef}
              className="fixed top-0 w-full z-[51] overflow-hidden bg-foreground text-background"
              data-testid="news-bar"
            >
              {/* Fade edges */}
              <div
                className="pointer-events-none absolute top-0 start-0 bottom-0 w-14 z-10"
                style={{
                  background:
                    "linear-gradient(to right, hsl(var(--foreground)), transparent)",
                }}
              />
              <div
                className="pointer-events-none absolute top-0 end-0 bottom-0 w-14 z-10"
                style={{
                  background:
                    "linear-gradient(to left, hsl(var(--foreground)), transparent)",
                }}
              />
              ...
              <motion.div
                ...
                transition={{ repeat: Infinity, duration: newsBarDuration, ease: "linear" }}
              >
                {[0, 1].map((g) => (
                  <span
                    key={g}
                  ...
                  >
                    {Array.from({ length: 4 }).map((_, rep) =>
                      items.map((item, i) => (
                        ...
                        >
                          <span className="text-xs sm:text-sm font-light tracking-[0.25em] uppercase text-background/80">
                            {item}
                          </span>
                          <span className="text-background/30 text-[8px]">
                            ✦
                          </span>
                        </span>
                      )),
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              {user ? (
                <div className="flex items-center gap-3 sm:gap-4">
                  ...
                  <ClickIcon animation="rubber" tag="span">
                    <Link
                      href="/wishlist"
                      className="relative hover:opacity-70 transition-opacity block"
                  ...
                  {/* Logout — wiggle goodbye — desktop only, mobile uses the menu */}
                  <span className="hidden sm:contents">
                    <ClickIcon
                      animation="wiggle"
                      onClick={() => logout.mutate()}
                      className="hover:opacity-70 transition-opacity bg-transparent border-none outline-none"
              ...
              {/* Search & language — grouped together at the far right, right next to the menu toggle */}
              <div className="flex items-center gap-2 sm:gap-4 ps-2 sm:ps-3 ms-1 border-s border-border/40">
                <ClickIcon
                  animation="pop"
                ...
                <ClickIcon
                  ...
                  className="flex items-center gap-1 text-sm hover:opacity-70 transition-opacity bg-transparent border-none outline-none"
        ...
        {mobileMenuOpen && (
          <div className="md:hidden absolute top-20 inset-x-0 w-full bg-background border-b shadow-lg text-foreground">
            <div className="flex flex-col p-4">
              <Link
                href="/"
                className="flex items-center gap-3 px-3 py-3 rounded-lg text-sm uppercase tracking-widest font-medium hover:bg-muted/50 transition-colors"
              ...
              <Link
                href="/sales"
                className="flex items-center gap-3 px-3 py-3 rounded-lg text-sm uppercase tracking-widest font-medium hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors"
                style={{ color: "#9B1C1C" }}
                onClick={() => setMobileMenuOpen(false)}
              >
                <BadgePercent
              ...
              {(user?.role === "admin" || user?.role === "employee") && (
                <Link
                  href="/admin"
                  className="flex items-center gap-3 px-3 py-3 rounded-lg text-sm uppercase tracking-widest font-medium text-primary hover:bg-primary/5 transition-colors"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  <LayoutDashboard
                    className="w-5 h-5 shrink-0 text-muted-foreground"
              ...
              {user && (
                <div className="border-t border-border/40 mt-2 pt-2">
                  <button
                    ...
                    className="dark-red-zone flex items-center gap-3 px-3 py-3 rounded-lg text-sm uppercase tracking-widest font-medium text-muted-foreground hover:text-destructive hover:bg-destructive/5 transition-colors w-full text-start"
            ...
            >
              ...
              <div className="flex items-center gap-3 px-4 sm:px-10 py-4 border-b border-border">
                ...
                <input
                  ...
                  placeholder={
                    isAr
                      ? "ابحثي بالاسم، اللون، السعر..."
                      : "Search by name, color, brand..."
                  }
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {searchHistory.map((item) => (
                              <div
                                key={item}
                                className="flex items-center gap-1 pl-3 pr-1 py-1.5 rounded-full border border-border bg-muted/40 text-sm"
                              >
                                <button
                  ...
                  {searchQuery.trim() && searchResults.length === 0 && (
                    <div className="text-center py-16">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {isAr
                          ? "جربي كلمة أخرى أو لون أو ماركة"
                          : "Try a different keyword, color, or brand"}
                      </p>
                    </div>
                  )}
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                        {searchResults.map((p: any) => {
                          ...
                          const hasDiscount =
                            p.discountPrice &&
                            parseFloat(p.discountPrice) < parseFloat(p.price);
                          const displayName =
                          ...
                          const discountPct = hasDiscount
                            ...
                            : 0;
                          const totalVariants =
                            variants.length || legacyColors.length;
                          return (
                            <button
                              key={p.id}
                            ...
                            >
                              ...
                              >
                                ...
                                {hasDiscount && (
                                  <span className="absolute top-2 start-2 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-md" style={{backgroundColor:"#9B1C1C"}}>
                                    -{discountPct}%
                                  </span>
                                )}
                              </div>
                                ...
                                <div className="mt-auto pt-1.5">
                                  {hasDiscount ? (
                                    <div className="flex items-center gap-1.5 flex-wrap">
                                      <span className="text-sm font-bold" style={{color:"#9B1C1C"}}>
                                        ₪
                                        {parseFloat(p.discountPrice).toFixed(0)}
                                      </span>
