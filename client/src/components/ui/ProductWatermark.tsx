export function ProductWatermark({
  ...
  // All sizes use container-relative units so they scale with the card,
  // not the viewport — this fixes the "too big on small mobile cards" issue.
  //
  // thumb — side thumbnail strip (~80-100px containers)
  // sm    — product listing cards (2-col on phone ~195px, larger on desktop)
  // md    — main product image viewer (always large, fixed responsive px)
  // lg    — best-sellers / featured cards (varies a lot: ~120px phone → 350px desktop)

  const logoSize =
    size === "thumb"
      // Slightly bigger on mobile for the horizontal thumbnail strip
      ? "w-[52%] h-auto"
      : size === "sm"
        // Fixed — prevents blowup on narrow desktop listing cards
        ? "w-16 h-auto"
        : size === "lg"
          // Mobile: 42% of the small best-seller card; desktop lg+: 44%
          ? "w-[42%] h-auto lg:w-[44%]"
          : // md — main product image: bigger on mobile, large on sm+
            "w-32 h-auto sm:w-48 sm:h-[132px]";

  const textSize =
    size === "thumb"
      // Min 5px so text is just visible on tiny phone thumbnails
      ? "text-[clamp(5px,1.5cqw,7px)]"
      : size === "sm"
        ? "text-[8px]"
        : size === "lg"
          // Min 8px on mobile (was 4px = invisible); 14px fixed on desktop
          ? "text-[clamp(8px,1.8cqw,14px)] lg:text-[14px]"
          : // md — bigger text on mobile
            "text-[13px] sm:text-[17px]";

  // md uses the container type too so text scales on mobile
  const useContainer = size !== "md";

  return (
    <div
      className={`absolute inset-0 flex flex-col items-center justify-center pointer-events-none select-none z-20 [container-type:inline-size] ${className ?? ""}`}
      aria-hidden="true"
    >
      <div className="flex flex-col items-center gap-0.5">
        ...
        >
          ...
          <ellipse
            ...
            rx="40"
            ry="15"
            transform="rotate(13 160 196)"
            fill="#f06ee8"
          />

          {/* Brand mark — stroke adds thickness at all display sizes */}
          <g
...
-149 91 -5 38 38 61 173 92 38 9 200 -6 275 -25z"
        ...
        <span
          className={`text-[#1a1a1a] font-black tracking-[0.28em] uppercase ${textSize} leading-none`}
        >
          LUCERNE BOUTIQUE
        </span>
      </div>
