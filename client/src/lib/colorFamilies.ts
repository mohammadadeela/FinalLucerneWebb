export interface ColorMember {
  ...
  nameAr: string;
  hex: string;
  /** True for alternate-spelling/transliteration aliases (e.g. "دهبي" for
   *  "ذهبي") that should still match products by name but shouldn't be
   *  offered as their own separate swatch in admin color pickers. */
  aliasOnly?: boolean;
}
...
export interface ColorFamily {
  ...
  nameEn: string;
  hex: string;
  /** Optional CSS background (gradient/pattern) used instead of a flat hex
   *  for metallics and prints, where a single solid color misrepresents
   *  the swatch (e.g. gold, silver, animal print). */
  swatch?: string;
  /** When true, products with these colors are still recognized/translated,
   *  but the family (and its member colors) are hidden from the customer
   *  color filter (e.g. animal print). */
...
export const COLOR_FAMILIES: ColorFamily[] = [
  {
    ...
    members: [
      ...
      { nameEn: "Zebra Print", nameAr: "نقشة الحمار الوحشي", hex: "#3A3A3A" },
    ],
  },
  {
    key: "white",
  ...
  },
  {
    ...
    members: [
      ...
      { nameEn: "Gold", nameAr: "دهبي", hex: "#D4AF37", aliasOnly: true },
      { nameEn: "Antique Gold", nameAr: "ذهبي عتيق", hex: "#C5A028" },
      { nameEn: "Rose Gold", nameAr: "ذهبي وردي", hex: "#B76E79" },
    ],
  },
  {
    key: "silver",
    ...
    nameEn: "Silver",
    hex: "#C0C0C0",
    swatch: "linear-gradient(135deg, #F4F4F4 0%, #C0C0C0 45%, #8A8A8A 100%)",
    members: [
      { nameEn: "Silver", nameAr: "فضي", hex: "#C0C0C0" },
      { nameEn: "Silver", nameAr: "سلفر", hex: "#C0C0C0", aliasOnly: true },
      { nameEn: "Metallic Gunmetal", nameAr: "رصاصي ميتاليك", hex: "#71797E" },
      { nameEn: "Platinum", nameAr: "بلاتيني", hex: "#E5E4E2" },
    ],
  },
  {
    key: "neutral",
    ...
    members: [
      ...
      { nameEn: "Sugar White", nameAr: "سكري", hex: "#FBFAF5" },
    ],
  },
  {
    key: "yellow",
  ...
  },
  {
    ...
    members: [
      ...
      { nameEn: "Fuchsia", nameAr: "فوشي", hex: "#FF00FF", aliasOnly: true },
      { nameEn: "Dusty Pink", nameAr: "زهري", hex: "#DDA0A0" },
      { nameEn: "Light Dusty Pink", nameAr: "زهري فاتح", hex: "#E8C4C4" },
    ],
  },
  {
    key: "purple",
  ...
  },
  {
    ...
    hex: "#06b6d4",
    members: [
      { nameEn: "Turquoise", nameAr: "تركوازي", hex: "#06b6d4" },
      { nameEn: "Turquoise", nameAr: "تركواز", hex: "#06b6d4", aliasOnly: true },
      { nameEn: "Teal Blue", nameAr: "تركوازي غامق", hex: "#0e7490" },
    ],
  },
  {
    key: "green",
    ...
    members: [
      ...
      { nameEn: "Snake Print", nameAr: "نقشة الأفعى", hex: "#8A9A7A" },
    ],
  },
  {
    key: "brown",
    ...
    members: [
      ...
      { nameEn: "Leopard Print", nameAr: "نقشة الفهد", hex: "#C19A6B" },
      { nameEn: "Leopard Print", nameAr: "نقشة فهد", hex: "#C19A6B", aliasOnly: true },
      { nameEn: "Tiger Print", nameAr: "نقشة نمر", hex: "#C19A6B" },
      { nameEn: "Tiger", nameAr: "تايجر", hex: "#C19A6B" },
      { nameEn: "Animal Print", nameAr: "نقشة حيوانية", hex: "#C19A6B" },
    ],
  },
  {
    key: "gray",
    ...
    members: [
      ...
      { nameEn: "Gunmetal", nameAr: "رصاصي", hex: "#6E7A82" },
    ],
  },
...
export function groupColorsByFamily(
  ...
  for (const c of colors) {
    ...
    if (family) {
      // Families flagged hideInFilters (e.g. animal print) are intentionally
      // excluded from the customer color filter — and their member colors
      // must NOT fall through to the ungrouped list either.
      if (family.hideInFilters) continue;
  ...
  // Canonical display order: core colors first (including gold & silver as
  // main colors), then chromatics light → dark.
  const FAMILY_ORDER = [
    "black", "white", "gold", "silver", "neutral", "yellow", "orange",
    "red", "pink", "purple", "blue", "turquoise", "green", "brown", "gray",
  ];
  const orderOf = (key: string) => {
    const i = FAMILY_ORDER.indexOf(key);
    return i === -1 ? FAMILY_ORDER.length : i;
  };
  const sortedFamilies = Array.from(grouped.values()).sort(
    (a, b) => orderOf(a.familyKey) - orderOf(b.familyKey)
  );

  return [...sortedFamilies, ...ungrouped];
}
