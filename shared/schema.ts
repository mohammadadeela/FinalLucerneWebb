import { pgTable, text, serial, integer, boolean, timestamp, numeric, jsonb, uniqueIndex, index } from "drizzle-orm/pg-core";
...
export const products = pgTable("products", {
  ...
  subcategoryIds: integer("subcategory_ids").array().default([]),
  brand: text("brand"),
  ...
  subcategoryIdx: index("idx_products_subcategory_id").on(table.subcategoryId),
}));
...
export const orders = pgTable("orders", {
  ...
  creditUsed: numeric("credit_used"),
  createdAt: timestamp("created_at").defaultNow(),
...
export const wishlist = pgTable("wishlist", {
  ...
  productIdx: index("idx_wishlist_product_id").on(table.productId),
}));
...
export const posOrders = pgTable("pos_orders", {
  ...
  exchangeHistory: jsonb("exchange_history").$type<Array<{
    exchangedAt: string;
    returnedItems: Array<{ productId: number; name?: string; quantity: number; size?: string; color?: string; price?: string }>;
    replacementItems?: Array<{ productId: number; name?: string; quantity: number; size?: string; color?: string; price?: string }>;
    override?: boolean;
    byRole?: string;
  }>>().default([]),
  createdAt: timestamp("created_at").defaultNow(),
...
export const exchangeRequests = pgTable("exchange_requests", {
  ...
  orderIdx: index("idx_exchange_requests_order_id").on(table.orderId),
  userIdx: index("idx_exchange_requests_user_id").on(table.userId),
  orderItemIdx: index("idx_exchange_requests_order_item_id").on(table.orderItemId),
  productIdx: index("idx_exchange_requests_product_id").on(table.productId),
}));

export const insertExchangeRequestSchema = createInsertSchema(exchangeRequests).omit({
  id: true, createdAt: true, resolvedAt: true, status: true, adminNote: true, userId: true,
});
...
export const productEvents = pgTable("product_events", {
  ...
  productIdx: index("idx_product_events_product_id").on(table.productId),
  createdAtIdx: index("idx_product_events_created_at").on(table.createdAt),
}));
...
export const notifications = pgTable("notifications", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id).notNull(),
  type: text("type").notNull(), // "exchange_approved" | "exchange_denied"
  message: text("message").notNull(),
  messageAr: text("message_ar").notNull(),
  link: text("link").notNull().default("/profile"),
...
// ── Product Groups (for staging products together for AI autofill) ──────────
