...
export async function sendExchangeAdminNotification(details: {
  ...
  const sizeColorRow = (details.preferredSize || details.preferredColor)
  ...
  const html = `
    <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:560px;margin:0 auto;border:1px solid #eee;background:#fafafa;">
      <div style="padding:24px 28px 18px;border-bottom:1px solid #eee;">
        <h1 style="text-align:center;font-size:19px;letter-spacing:4px;margin:0 0 4px;">LUCERNE BOUTIQUE</h1>
        <p style="text-align:center;color:#888;font-size:12px;margin:0;">لوسيرن بوتيك</p>
      </div>
  ...
  if (!t) {
    console.log(`[email] FALLBACK — Exchange admin notification for order #${details.orderId}`);
    return;
  }
  ...
  try {
    ...
    console.log(`[email] Exchange admin notification sent for order #${details.orderId}`);
  } catch (err) {
...
export async function sendAbandonedCartEmail(to: string, customerName: string): Promise<void> {
  const t = getTransporter();
  ...
  const html = `
    <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:520px;margin:0 auto;border:1px solid #eee;background:#fafafa;">
      <div style="padding:24px 28px 18px;border-bottom:1px solid #eee;text-align:center;">
        <h1 style="font-size:20px;letter-spacing:4px;margin:0 0 4px;">LUCERNE BOUTIQUE</h1>
        <p style="color:#888;font-size:12px;margin:0;">لوسيرن بوتيك</p>
      </div>
      <div dir="rtl" style="padding:24px 28px;">
        ...
        <div style="text-align:center;margin:20px 0;">
          <a href="https://lucerneboutique.com/cart" style="display:inline-block;padding:12px 32px;background:#111;color:#fff;text-decoration:none;font-size:14px;border-radius:4px;letter-spacing:1px;">
            إتمام الطلب
          </a>
        </div>
  ...
  if (!t) {
    console.log(`[email] FALLBACK — Abandoned cart email to ${redactEmail(to)}`);
    return;
  }
...
export async function sendSaleDiscountEmail(
  ...
  for (const recipient of recipients) {
    const html = `
      <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:520px;margin:0 auto;border:1px solid #eee;background:#fafafa;">
        ...
        <div dir="rtl" style="padding:0 28px 20px;">
          ...
          ${categoryBlock}
          <div style="text-align:center;margin:20px 0;">
            <a href="https://lucerneboutique.com" style="display:inline-block;padding:12px 32px;background:#111;color:#fff;text-decoration:none;font-size:14px;border-radius:4px;letter-spacing:1px;">
              تسوقي الآن
            </a>
          </div>
    ...
    if (!t) {
      console.log(`[email] FALLBACK — Sale email to ${redactEmail(recipient.email)}`);
      continue;
    }
    ...
    try {
      ...
      console.error(`[email] Failed to send sale email to ${redactEmail(recipient.email)}:`, err);
    }
  }
}
...
export async function sendDiscountCodeEmail(
  ...
  const remaining = details.maxUses != null
    ? Math.max(0, details.maxUses - (details.usedCount ?? 0))
    : null;

  const expiryStr = details.expiresAt
    ? details.expiresAt.toLocaleString("ar-SA", { dateStyle: "long", timeStyle: "short" })
    : null;
  ...
  const usesBlock = remaining != null
  ...
  for (const recipient of recipients) {
    const html = `
      <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:520px;margin:0 auto;border:1px solid #eee;background:#fafafa;">
        ...
        <div dir="rtl" style="padding:0 28px 24px;">
          ...
          ${usesBlock}
          <div style="text-align:center;margin:24px 0;">
            <a href="https://lucerneboutique.com" style="display:inline-block;padding:13px 36px;background:#111;color:#fff;text-decoration:none;font-size:14px;border-radius:4px;letter-spacing:1px;">
              تسوقي الآن · Shop Now
            </a>
          </div>
          <hr style="border:none;border-top:1px solid #eee;margin:16px 0;"/>
    ...
    if (!t) {
      console.log(`[email] FALLBACK — Discount code email to ${redactEmail(recipient.email)}`);
      continue;
    }
    ...
    try {
      await t.sendMail({
        from: `"Lucerne Boutique" <${getSenderEmail()}>`,
        to: recipient.email,
        subject: `🎁 كود خصم خاص بكِ ${details.discountPercent}% · ${details.code} — Lucerne Boutique`,
        html,
      });
      console.log(`[email] Discount code email sent to ${redactEmail(recipient.email)}`);
    } catch (err) {
      console.error(`[email] Failed to send discount code email to ${redactEmail(recipient.email)}:`, err);
    }
  }
}
